...
const Select = () => {
  ...
  const handleAvailableChains = (location) => {
    switch (location) {
      ...
      case "/newmessage":
        setAvailableChains(checkNetworkSupport("messenger"));
        break;
      case "/invest/buy_token":
        setAvailableChains(checkNetworkSupport("lottIo"));
        break;
      case "/invest/referral":
        setAvailableChains(checkNetworkSupport("lottWhiteList"));
        break;
      default:
        break;
    }
  ...
  const checkNetworkSupport = (contractName) => {
    ...
    for (let key in data.addresses) {
      if (
        data.addresses[key][contractName] &&
        data.addresses[key][contractName].length !== 0
      )
        tempChains.push({ chain: key, supported: true });
      else tempChains.push({ chain: key, supported: false });
    }
    return tempChains;
  };
  useEffect(() => {
          ...
          <>
            {data.network ? (
              <span style={{ textTransform: "capitalize" }}>
                {data.network}
                {!supported && <Badge bg="danger">Not supported</Badge>}
              </span>
            ) : (
              "not connected"
            )}
