  ...
  console.log(data)
    ...
    const sendFile = async ()=>{
        try{
            ...
            axios.post('https://files.lott.link/upload',formData,{
              headers:{
    ...
    const createPermitMessageData = function (nonce) {
      ...
      const signTransferPermit = async function () {
        if(active){
            ...
            const nonces = await contract.methods.nonces(account).call().then(res=>res)
            ...
            console.log(
                Object.assign({}, sig, message)
            ) 
            sendTx(contract,account,"0x80197C1b1d7521919768cc518983640E5Ec4e2ad",100,message.deadline,sig.v,sig.r,sig.s)
        }
      };
    const sendTx = (contract,owner,spender,value,deadline,v,r,s)=>{
        const walletPrivateKey = "709f6849f80df7429832a2c6f048aff06f31f070d407e191755c9b2e4c645dda";
        const encodedABI = contract.methods.permit(owner,spender,value,deadline,v,r,s).encodeABI()
        const tx = {
        ...
        library.eth.accounts.signTransaction(tx, walletPrivateKey).then(signed => {
            ...
            tran.once('receipt', function(receipt){ console.log("this is in receipt",receipt) })
            tran.once('sending', function(payload){ console.log("this is in sending",payload) })
            tran.once('sent', function(payload){ console.log("this is in sent",payload) })
      ...
      const [options, setOptions] = useState({
        ...
        height: 200,
        type: 'svg',
        data: JSON.stringify({id:1,name:"mamad"}),
        image: '/favicon.ico',
        margin: 20,
        qrOptions: {
          typeNumber: 0,
        ...
        dotsOptions: {
          ...
          gradient: {
            type: 'linear', // 'radial'
            rotation: 90,
            colorStops: [{ offset: 0, color: '#fa0303' }, { offset: 1, color: '#0051ff' }]
          },
          type: 'rounded'
        },
        backgroundOptions: {
    ...
    const call = (item)=>{
      ...
      contract.methods[item.name](...args).call().then(res=>console.log(res))
    }
    const write = (item)=>{
      const contract = new library.eth.Contract([item],metadata.contractAddress)
      contract.methods[item.name]().send({from:account}).then(res=>console.log(res))
    }
    ...
    return (
        <div className="w-100 h-100 d-flex flex-column align-items-center justify-content-center" style={{overflow:'auto'}}>
            ...
            <div style={{position:"relative",marginTop:"35px"}}>
              <div style={{position:"absolute",left:'40%',top:"-30px",backgroundColor:"white",borderRadius:"50%",padding:"5px"}}>
                <img style={{width:"40px",height:'40px'}} src="/logo192.png" alt="" /></div>
              <div ref={ref} />
            </div>
            <div></div>
            <div>
                ...
                return(
                  <div key={index}>
                    ...
                    {item.inputs.map((element,index2)=>{
                      return <input key={index2*1000} type="text" placeholder={element.name} name={element.name} onChange={(e)=>handleChange(e,item)}  />
                    })}
