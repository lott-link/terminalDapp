...
const TransferNFT = ({
	...
	const transfer = async () => {
		...
		const contract = new library.eth.Contract(
			abi,
			data.addresses[data.network]["crossChain"]
		);
		contract.methods
			...
			.send({ from: account, value: parseInt(fee) })
	...
	const release = () => {
		...
		contract.methods
			.requestReleaseLockedToken(selectedToken.tokenID, address, account)
			.send({ from: account, value: parseInt(fee) })
			.on("transactionHash", (transactionHash) => {
				setTransferBtn({ ...transferBtn, loading: true, approving: false });
			})
	...
	const handleTarget = (e) => {
		setTargetChain(data.chains[e.target.value]["chainIdHex"]);
	...
	const mintFee = async () => {
		...
		const tempFee = await contract.methods
			.mintFee(data.chains[data.converChainIDToName(targetChain)]["chainIdHex"])
			.call();
		setFee(tempFee);
	};
	const setTarget = () => {
		if (selectedWay === false) {
			...
			contract.methods
				.getLockedTokenData(selectedToken.tokenID)
				.call()
				.then((res) => {
					console.log(res);
			...
			data.network && setTargetChain(data.chains[data.network]["chainIdHex"]);
		}
	};
	...
	return (
		<div
			className={`${width < 992 ? "w-100" : "w-50"} p-2 mx-auto ${
				styles["animation-in"]
			}`}
			style={{
				height: "100vh",
		...
		>
			<div className="w-100 h-100" style={{ border: "1px solid white" }}>
				<div className={`py-3 text-center ${width > 992 ? "px-4" : "px-2"}`}>
					<div>select your destination network</div>
					<div className={`text-center ${width > 992 && "m-3"}`}>
						<select
							name=""
							className={`${styles.select} p-1 text-center`}
				...
				<div className="text-center text-dark">
					<div
						className="bg-white d-flex justify-content-around align-items-center "
						style={{ margin: "0 40px" }}
					>
						<div className="d-flex ">
							<OverlayTrigger
								placement={"bottom"}
								overlay={<Tooltip>{fee} wei</Tooltip>}
							>
				...
				<div className="text-center" style={{ padding: "0 48px" }}>
					your NFT will appear in your destination address in a few minutes
				</div>
			</div>
